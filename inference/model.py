...
def linear(x: torch.Tensor, weight: torch.Tensor, bias: Optional[torch.Tensor] = None, scale_fmt: Optional[str] = None) -> torch.Tensor:
    ...
    if weight.element_size() > 1:
        ...
        x, scale = act_quant(x, block_size, scale_fmt) #need

        y = fp8_gemm(x, scale, weight, weight.scale) #need
        #if count is not None:
            # torch.save({"bf16input": x, "fp32scale": scale, "block_size":block_size}, f"./case/fp8quant_{count}.pt")
            # torch.save({"fp8input": x, "bf16output": y, "fp32scale": scale, "weight": weight, "weight_scale":weight.scale}, f"./case/matrix_mul_{count}.pt")
        if bias is not None:
            y += bias
        return y
...
class Linear(nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, bias: bool = False, dtype = None):
        ...
        if self.weight.element_size() == 1:
            ...
            self.weight.scale = self.weight_scale_inv = nn.Parameter(torch.empty(scale_out_features, scale_in_features, dtype=torch.float32))
        else:
...
class MLA(nn.Module):
    ...
    def forward(self, x: torch.Tensor, start_pos: int, freqs_cis: torch.Tensor, mask: Optional[torch.Tensor]):
        ...
        if attn_impl == "naive":
            ...
            kv_b_proj = self.kv_b_proj.weight if self.kv_b_proj.weight_scale_inv is None else weight_dequant(self.kv_b_proj.weight, self.kv_b_proj.weight_scale_inv, block_size) 
            kv_b_proj = kv_b_proj.view(self.n_local_heads, -1, self.kv_lora_rank)
            q_nope = torch.einsum("bshd,hdc->bshc", q_nope, kv_b_proj[:, :self.qk_nope_head_dim])
...
class Gate(nn.Module):
    ...
    def __init__(self, args: ModelArgs):
        ...
        self.e_score_correction_bias = nn.Parameter(torch.empty(args.n_routed_experts, dtype=torch.float32)) if self.dim == 7168 else None
...
class Block(nn.Module):
    ...
    def __init__(self, layer_id: int, args: ModelArgs):
        ...
        self.mlp = MLP(args.dim, args.inter_dim) if layer_id < args.n_dense_layers else MoE(args)
...
class Transformer(nn.Module):
    ...
    def __init__(self, args: ModelArgs):
        ...
        self.lm_head = ColumnParallelLinear(args.dim, args.vocab_size, dtype=torch.get_default_dtype())
